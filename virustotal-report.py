...
#Loop through each line of shortpees.txt
for l in f:
    l = l.rstrip()
    ...
    for k,v in data.items(): # For loop for JSON result, 
        try:
            ...
            [l, 
            now, 
            json.dumps(data['scan_date']), 
            json.dumps(data['scan_id']), 
            json.dumps(data['permalink']), 
            json.dumps(data['filescan_id']), 
            json.dumps(data['positives']), 
            json.dumps(data['total']), 
            json.dumps(data['scans']['BitDefender']), 
            json.dumps(data['scans']['Kaspersky']), 
            json.dumps(data['scans']['Spamhaus']), 
            json.dumps(data['verbose_msg'])])
